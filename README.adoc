...
This guide walks you through the process of creating a Spring Boot application that uses Grails Object Relational Mapper (http://grails.org/doc/latest/guide/GORM.html[GORM]) and http://www.mongodb.org[MongoDB] for persistence. The application uses to http://springsource.github.io/grails-data-mapping/mongo/manual/index.html[GORM for MongoDB plugin].

== What you'll build

You'll build a Spring application that persists geospacial data to MongoDB using GORM. 
...
The http://springsource.github.io/grails-data-mapping/mongo/manual/guide/3.%20Mapping%20Domain%20Classes%20to%20Mongo%20Collections.html#3.2%20Indexing%20Queries[mapping block] specifies that the `location` property should be indexed using a http://docs.mongodb.org/manual/core/2dsphere/[geospacial 2sphere] index.
